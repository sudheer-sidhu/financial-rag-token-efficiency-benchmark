# compare_json_vs_toon_tiktoken_v2.py
...
# Install: python -m pip install git+https://github.com/toon-format/toon-python.git
...
def build_chunks_as_objects(doc_id: str, cfg: CompareConfig, chunks: list[str]) -> list[dict]:
    """
    Build ONE canonical Python representation of the corpus.
    This same object list is serialized as JSON and as TOON (fair comparison).
    """
    base_meta = {
        "ticker": cfg.ticker,
        "form": cfg.form,
    ...
    for i, ch in enumerate(chunks):
        ...
        char_cursor = char_end + 1  # +1 for a separator/newline conceptually
...
def main() -> None:
    ...
    for p in files:
        ...
        chunk_objs = build_chunks_as_objects(p.stem, cfg, chunks)

        # JSON serialization (single corpus string)
        json_corpus = json.dumps(chunk_objs, ensure_ascii=False)

        #  TOON serialization (single corpus string)
        toon_corpus = toon_encode(chunk_objs)

        #  Optional correctness check ( TOON round-trip)
        # If this fails,  TOON is not legit or object types are not supported.
        decoded = toon_decode(toon_corpus)
        if decoded != chunk_objs:
            raise SystemExit(
                f"[FAIL] TOON round-trip mismatch for {p.name}. "
                "This means encoding/decoding isn't preserving the object exactly."
            )
